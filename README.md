...
[![GitHub license](https://img.shields.io/github/license/shaggydogstudios/VYaml-Net)](./LICENSE)

VYaml-Net is a pure C# YAML 1.2.2 implementation which is very fast with a low memory footprint and minimal dynamic allocations. It is focused on modern .NET and C# requiring .NET 11 (preview) at time of writing.

It is a fork of the excellent [VYaml](https://github.com/hadashiA/VYaml) library and all credit goes to [hadashiA](https://github.com/hadashiA) for implementing it. This fork only differs slightly from upstream:
- Unity compatibility is dropped
  - This allows updating all package dependencies to latest versions fixing all known security vulnerabilities.
  - It also allows deleting some unity shims / polyfill and general 'old versions' support throughout the project.
- It replaces ReadOnlySequence usage with ReadOnlySpan in the tokenizer.
  - This trades off the capability of chunked streaming of large files with simpler logic and less memory indirections.
- The combination of updated runtime, dependencies and simplified data access makes the deserializer about 12% faster in this fork.

**NOTE:** Most people will want to use the original [VYaml](https://github.com/hadashiA/VYaml) library - this fork primarily exists for our own use, no promise of maintenance is implied.
...
- YAML Parser (Reader)
  - [YAML 1.2.2 mostly supported](#httpsyamlorgspec122)
...
- Deserialize / Serialize
  ...
  - Customization
    - Rename key
    - Ignore member
- NOT focused on Unity (use the original [VYaml](https://github.com/hadashiA/VYaml) library there)
    - Requires .NET 11 and C# 14 which Unity does not support
...
https://www.nuget.org/packages/VYaml-Net

```bash
dotnet add package VYaml-Net
```
...
Original author: [@hadashiA](https://github.com/hadashiA)
Branch author: [@tlaedre](https://github.com/tlaedre)
